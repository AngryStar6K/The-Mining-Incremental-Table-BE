...
function hyperEformat(decimal, precision) {
    ...
    if (decimal.layer >= 1e9.toNumber()) return hyperEformat(s, precision) + '#2'
    // if (decimal.layer >= 1e9) return 'E1#'+e
    if ((decimal.layer > 0 && decimal.mag > 0) || decimal.mag >= 1e9.toNumber()) return 'E' + m + '#' + e
    if (decimal.mag >= 1e3) return commaFormat(decimal, 0)
    if (decimal.mag >= 0.001) return regularFormat(decimal, precision)
...
var format = f = function (decimal, precision = 3) {
    ...
    else if (options.notation == 'Emoji') {
        return letter(decimal, precision, ['😠', '🎂', '🎄', '💀', '🍆', '🐱', '🌈', '💯', '🍦', '🎃', '💋', '😂', '🌙', '⛔', '🐙', '💩', '❓', '☢', '🙈', '👍', '☂', '✌', '⚠', '❌', '😋', '⚡'])
    }
    else if (options.notation == 'BEAF') {
...
var formatPercent = fp = function(decimal, boost) {
    decimal = new Decimal(decimal)
...
function formatTime(ms) {
    let Ds = new Decimal(ms).div(1000)
    let Ns = Ds.toNumber()
...
var formatTimeLong = ftl = function (s) {
    s = new Decimal(s)
...
var formatExtremeSmallNum = fesn = function (num, precision = 2) { //伪装成极小数，ExpantaNum.js不支持像break_eternity.js那样的绝对值不为0且小于5e-324的极小数
    ...
    if (num.abs().gte(1000) && num.abs().lt("10^^5")) return format(num, precision) + "⁻¹"
